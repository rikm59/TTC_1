...
export default function MilestoneEditor({ milestones, totalQuote, onChange }: Props) {
  ...
  const update = (id: string, field: keyof PaymentMilestone, value: string | number) => {
        ...
        <div className="space-y-2">
          {milestones.map((m, i) => (
            <div key={m.id} className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
              ...
              <input
                className="flex-1 min-w-0 text-xs border border-gray-200 rounded px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-brand-300"
                value={m.label}
              ...
              <div className="flex items-center gap-0.5 shrink-0">
                <input
                  ...
                  min={1}
                  max={100}
                  className="w-14 text-right text-xs border border-gray-200 rounded px-1.5 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-brand-300"
                  value={m.percent}
                  onChange={e => update(m.id, 'percent', Math.max(1, Math.min(100, +e.target.value)))}
                />
                <span className="text-gray-400 text-xs">%</span>
              </div>
              <span className="text-xs text-brand-700 font-medium w-20 text-right shrink-0">
                {fmt(totalQuote * m.percent / 100)}
              </span>
              <input
                className="w-28 text-xs border border-gray-200 rounded px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-brand-300 shrink-0"
                value={m.dueOn}
          ...
          {/* Total indicator */}
          <div className={`flex justify-end text-xs font-medium px-3 ${isBalanced ? 'text-green-600' : 'text-amber-600'}`}>
            {isEs ? 'Total:' : 'Total:'} {totalPct.toFixed(0)}%
            {!isBalanced && ` (${isEs ? 'debe sumar 100%' : 'must total 100%'})`}
          </div>
        </div>
      )}
